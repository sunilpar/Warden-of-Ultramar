 ...
 */
const TOTAL_W = 240;
const LEFT_RATIO = 0.7; // 70% for the text column
const PADDING_X = 10; // left/right inset inside the panel
const PADDING_Y_TOP = 18; // top padding (breathing room above title)
const PADDING_Y_BOT = 18; // bottom padding (breathing room below last mod)
...
export function buildCardTooltipPanel(
  ...
  const contentH =
    PADDING_Y_TOP + // top padding (was PADDING_Y)
    14 + // title row
    TITLE_GAP + // gap
    modLines.length * LINE_GAP + // mod rows
    PADDING_Y_BOT; // bottom padding (was PADDING_Y)
