...
export class GameRoom2 extends Room {
    constructor() {
        ...
        this.activeModifiers = MAP_MODIFIERS["game_room_2"] ?? [];
        ...
        this.messages = {
            ...
            5: (client, _msg) => {
                ...
                if (!player)
                    return;
                player.addXp(1000);
                console.log(`Player ${client.sessionId} earned 1000 XP for map transition (map2 -> map3)`);
            },
            ...
            7: (client, msg) => {
                ...
                const card = player.slotCard(slot);
                if (!card || card.level >= MAX_SKILL_LEVEL)
                    return;
