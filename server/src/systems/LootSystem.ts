...
export class LootSystem {
  ...
  rollEnemyCard(enemy: Enemy): CardInstance | null {
    ...
    if (Math.random() > CARD_DROP.SPAWN_WITH_CARD) return null;
  ...
  private rollEnemyCardUngated(enemy: Enemy): CardInstance | null {
    ...
    if (rarity === "unique") {
      const allowed = UNIQUE_POOL.filter((u) => u.appliesTo.includes(skill));
      if (allowed.length === 0) return this.rollEnemyCardUngated(enemy);
      card.modIds = new ArraySchema<string>(allowed[0].id);
      card.rarity = "unique";
      return card;
    }
