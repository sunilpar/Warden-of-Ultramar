...
export class Player extends Schema {
  ...
  private moveSlotCooldown(a: number, b: number): void {
  ...
  private shiftSlotCooldowns(from: number, to: number, dir: 1 | -1): void {
    if (dir === 1) {
      for (let i = to; i >= from; i--) this.moveSlotCooldown(i, i + 1);
    } else {
      for (let i = to; i <= from; i++) this.moveSlotCooldown(i, i - 1);
    }
  }
   ...
   */
  upgradeSlotCard(i: number): boolean {
    const c = this.slotCard(i);
    if (!c) return false;
   ...
   * empty `from` slot.
   */
  moveSlotCard(from: number, to: number): boolean {
    if (from === to) return false;
    if (from < 0 || from >= NUM_CARD_SLOTS) return false;
    if (to < 0 || to >= NUM_CARD_SLOTS) return false;
