...
/**
 * Per-mod value ranges by tier. Fraction-style mods (crit rate, damage,
 * cooldown) are stored as fractions: 0.01..0.05 == 1%..5%.
 * crit damage is stored as a fraction too: 0.1 == +10% crit damage.
 */
...
export type ModTierRanges = Record<number, ModRange>;

export const MOD_TIER_RANGES: Record<string, ModTierRanges> = {
 ...
 */
export function rollModValue(modId: string, tier: number): number {
  const ranges = MOD_TIER_RANGES[modId];
  if (!ranges) return 1; // unique/unknown: neutral placeholder
