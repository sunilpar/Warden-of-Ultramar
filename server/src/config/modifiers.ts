...
export interface ModifierDef {
  ...
  title: string;
  description: string;
  /** Apply effect to a player (called on join/respawn). */
  applyPlayer?: (player: Player) => void;
  /** Apply effect to an enemy (called on spawn). */
  applyEnemy?: (enemy: Enemy) => void;
  /** Multiplier for enemy xpReward. */
  xpRewardMultiplier?: number;
  /** Flat bonus to loot rarity roll. */
...
export function getLootRarityBonus(activeIds: ModifierId[]): number {
  let bonus = 0;
  for (const id of activeIds) {
    const def = MODIFIER_DEFS[id];
...
export function getModifierInfo(activeIds: ModifierId[]): ModifierDef[] {
